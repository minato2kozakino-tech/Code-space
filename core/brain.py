...
class MarsBrain:
    def __init__(self, cfg):
        self.cfg = cfg
        self.model_path = cfg['paths']['gaton_model'] # Updated model path
        ...
        self.tfidf = {}
        self.vectorizer = None # Scikit-learn TF-IDF Vectorizer
        self.lang_classifier = None # Scikit-learn Language Classifier

        # Markov Chain (will be initialized and managed by trainer/engine)
        ...
        if os.path.exists(path):
            try:
                with open(path, "rb") as f:
                    ...
                    self.lang_classifier = data.get("lang_classifier", None)
                    self.markov_model = data.get("markov_model", None)
    ...
    def expand(self, words):
        ...
        if not self.W_h.any(): # Initialize W_h only if it's empty
    ...
    def save(self):
        with open(self.model_path, "wb") as f:
            pickle.dump({
                ...
                "tfidf": self.tfidf, # Old TF-IDF
                "vectorizer": self.vectorizer, # Scikit-learn Vectorizer
                "lang_classifier": self.lang_classifier, # Scikit-learn Classifier
                "markov_model": self.markov_model # Markov Chain Model
            }, f)
